...
CREATE TABLE `bls_member_user` (
                                   ...
                                   `user_id` bigint(20) NOT NULL COMMENT '用户ID',
                                   `level_code` varchar(32) COMMENT '当前等级编码',
...
CREATE TABLE `bls_points_rule` (
                                   ...
                                   `scene` tinyint(2) NOT NULL COMMENT '积分场景：获取场景：1-消费 2-签到 3-活动 4-评价 5-首次绑定 6-邀请好友；消耗场景：21-抵扣 22-兑换商品 23-兑换优惠券',
...
CREATE TABLE `bls_member_points_record` (
                                            ...
                                            `type` tinyint(1) NOT NULL COMMENT '类型：1-增加 2-消费 3-过期',
...
CREATE TABLE `bls_pay_channel_config` (
  ...
  `is_delete` tinyint DEFAULT 0 COMMENT '删除标志（0代表存在 1代表删除）',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_pay_store` (`store_id`)
...
-- 初始化会员等级配置数据
INSERT INTO `bls_member_level_config` (`id`, `level_code`, `level_name`, `required_amount`, `discount`, `monthly_free_minutes`,
                                   `points_multiplier`, `birthday_discount`, `friend_privilege_count`, `vip_service`, `reservation_privilege`, `description`)
VALUES
    ('1', 'normal', '普通会员', 0, 0.95, 0, 1.00, NULL, 0, 0, 0, '入门级别会员，享受9.5折优惠'),
    ('2', 'silver', '银牌会员', 1000, 0.90, 0, 1.20, 0.85, 0, 0, 1, '中级会员，享受9折优惠，生日特惠8.5折'),
    ('3', 'gold', '金牌会员', 3000, 0.85, 120, 1.50, 0.80, 1, 1, 1, '高级会员，享受8.5折优惠，每月赠送2小时'),
    ('4', 'diamond', '钻石会员', 10000, 0.80, 300, 2.00, 0.75, 2, 1, 1, '至尊级别，享受8折优惠，每月赠送5小时，可带2位朋友享会员价');

-- 初始化基础积分规则
INSERT INTO `bls_points_rule` (`id`, `name`, `type`, `scene`, `value_type`, `points_value`, `description`, `status`)
VALUES
    ('1', '消费积分', 1, 1, 1, 1.00, '每消费1元获得1积分', 1),
    ('2', '每日签到', 1, 2, 1, 5.00, '每日签到获得5积分', 1),
    ('3', '首次绑定', 1, 5, 1, 100.00, '首次绑定手机号获得100积分', 1),
    ('4', '邀请好友', 1, 6, 1, 50.00, '成功邀请好友获得50积分', 1),
    ('5', '积分抵扣', 2, 1, 1, 0.01, '100积分抵扣1元', 1);
