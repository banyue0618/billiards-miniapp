...
import org.dromara.common.pay.enums.AuthTypeEnum;
...
public class WxPayKit {
     ...
     * 验证签名
     *
     * @param response 接口请求返回的 {@link PayHttpResponse}
     * @param certPath 平台证书路径
     * @return 签名结果
     * @throws Exception 异常信息
     */
    public static boolean verifySignature(PayHttpResponse response, String certPath) throws Exception {
     ...
     * @throws Exception 异常信息
     */
    public static boolean verifySignature(PayHttpResponse response, InputStream certInputStream) throws Exception {
        String timestamp = response.getHeader("Wechatpay-Timestamp");
        String nonceStr = response.getHeader("Wechatpay-Nonce");
        String signature = response.getHeader("Wechatpay-Signature");
        String body = response.getBody();
