...
import org.dromara.common.pay.enums.PayConstants;
import org.dromara.common.pay.enums.RequestMethodEnum;
import org.dromara.common.pay.model.CertificateModel;
import org.dromara.common.pay.util.XmlHelper;
...
public class PayKit {
    ...
    public static boolean checkCertificateIsValid(CertificateModel model, String mchId, int offsetDay) {
        ...
        if (null == issuerDn || !issuerDn.getName().contains(PayConstants.ISSUER)) {
            return false;
        }
        // 证书CN字段
        if (StrUtil.isNotEmpty(mchId)) {
            Principal subjectDn = model.getSubjectDn();
            if (null == subjectDn || !subjectDn.getName().contains(PayConstants.CN.concat(mchId.trim()))) {
                return false;
            }
        }
        // 证书序列号固定40字节的字符串
        String serialNumber = model.getSerialNumber();
        if (StrUtil.isEmpty(serialNumber) || serialNumber.length() != PayConstants.SERIAL_NUMBER_LENGTH) {
            return false;
        }
        // 偏移后的时间
