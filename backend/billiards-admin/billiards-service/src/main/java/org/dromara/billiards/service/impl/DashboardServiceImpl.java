...
public class DashboardServiceImpl implements DashboardService {
    ...
    public DashboardOverviewVO getOverviewData(DashboardQueryRequest request) {
        ...
        try {
            ...
            // 获取今日和昨日的营收数据
            vo.setTodayRevenue(dashboardMapper.calculateDailyRevenue(today, request.getStoreIds(), MerchantHolder.get()));
            vo.setYesterdayRevenue(dashboardMapper.calculateDailyRevenue(yesterday, request.getStoreIds(), MerchantHolder.get()));

            // 获取今日和昨日的订单数据
            vo.setTodayOrderCount(dashboardMapper.countDailyOrders(today, request.getStoreIds(), MerchantHolder.get()));
            vo.setYesterdayOrderCount(dashboardMapper.countDailyOrders(yesterday, request.getStoreIds(), MerchantHolder.get()));

            // 获取总桌台和使用中桌台数量
            vo.setTotalTables(dashboardMapper.countTotalTables(request.getStoreIds(), MerchantHolder.get()));
            vo.setActiveTables(dashboardMapper.countActiveTables(request.getStoreIds(), MerchantHolder.get()));

            // 获取今日和昨日的平均使用时长
            vo.setAvgUsageTime(dashboardMapper.calculateAverageUsageTime(today, request.getStoreIds(), MerchantHolder.get()));
            vo.setYesterdayAvgUsageTime(dashboardMapper.calculateAverageUsageTime(yesterday, request.getStoreIds(), MerchantHolder.get()));
...
//            vo.setYesterdayNewMembers(dashboardMapper.countDailyNewMembers(yesterday, request.getStoreIds()));
    ...
    public DashboardChartVO getDashboardChartData(DashboardQueryRequest request) {
        ...
        try {
            ...
            while (!currentDate.isAfter(request.getEndDate())) {
                ...
                // 获取营收数据
                revenueData.add(dashboardMapper.calculateDailyRevenue(currentDate, request.getStoreIds(), MerchantHolder.get()));

                // 获取订单数据
                orderData.add(dashboardMapper.countDailyOrders(currentDate, request.getStoreIds(), MerchantHolder.get()));

                // 获取桌台使用率
                tableUsageData.add(dashboardMapper.calculateDailyTableUsageRate(currentDate, request.getStoreIds(), MerchantHolder.get()));

                // 获取新增会员数
                newMemberData.add(dashboardMapper.countDailyNewMembers(currentDate, request.getStoreIds(), MerchantHolder.get()));
    ...
    public RevenueAnalysisVO getRevenueAnalysis() {
        ...
        try {
            ...
            // 获取当期和同期营收
            vo.setCurrentRevenue(dashboardMapper.calculateDailyRevenue(today, null, MerchantHolder.get()));
            vo.setPreviousRevenue(dashboardMapper.calculateDailyRevenue(today.minusYears(1), null, MerchantHolder.get()));
            ...
            vo.setMonthOverMonthGrowth(calculateGrowthRate(
                dashboardMapper.calculateDailyRevenue(today, null, MerchantHolder.get()),
                dashboardMapper.calculateDailyRevenue(today.minusMonths(1), null, MerchantHolder.get())
            ));
    ...
    public RevenueTrendVO getRevenueTrend(DashboardQueryRequest request) {
        ...
        try {
            ...
            while (!cur.isAfter(end)) {
                ...
                revenueSeries.add(dashboardMapper.calculateDailyRevenue(cur, request.getStoreIds(), MerchantHolder.get()));
                orderSeries.add(dashboardMapper.countDailyOrders(cur, request.getStoreIds(), MerchantHolder.get()));
