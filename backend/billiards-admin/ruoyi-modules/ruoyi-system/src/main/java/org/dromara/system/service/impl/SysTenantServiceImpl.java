...
public class SysTenantServiceImpl implements ISysTenantService {
     ...
     */
    @Override
    public List<SysTenantVo> searchByKeyword(String keyword, Integer limit) {
        String kw = StringUtils.trim(keyword);
        if (StringUtils.isBlank(kw)) {
            return Collections.emptyList();
        }
        LambdaQueryWrapper<SysTenant> lqw = Wrappers.lambdaQuery();
        lqw.and(w -> w.like(SysTenant::getCompanyName, kw)
            .or().like(SysTenant::getDomain, kw)
            .or().like(SysTenant::getTenantId, kw));
        lqw.orderByAsc(SysTenant::getId);
        if (limit != null && limit > 0) {
            Page<SysTenantVo> page = baseMapper.selectVoPage(new Page<>(1, limit), lqw);
