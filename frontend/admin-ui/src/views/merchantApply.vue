<template>
  <div class="apply">
    <el-card class="apply-card" shadow="hover">
      <h3 class="title">商户入驻申请</h3>
      <el-form ref="merchantApplyFormRef" :model="form" :rules="rules" label-width="110px" class="apply-form">
        ...
        <el-row :gutter="16">
          <el-col :span="12">
            <el-form-item label="Logo" prop="logo">
              <el-upload class="avatar-uploader" :http-request="handleLogoUpload" :show-file-list="false" :before-upload="beforeFileUpload">
                <img v-if="logoPreviewUrl" :src="logoPreviewUrl" class="avatar" />
...
import { addMerchantApply } from '@/api/system/merchantApply';
import { MerchantApplyForm } from '@/api/system/merchantApply/types';
...
const data = reactive<{ form: MerchantApplyForm; rules: ElFormRules }>({
  form: { ...initFormData },
  rules: {
...
async function handleLogoUpload(option: any) {
  const { file } = option;
  try {
    const res = await uploadFile(file, 'MERCHANT_LOGO');
    if (res && res.data) {
      form.value.logo = res.data.resourceId as any;
      logoPreviewUrl.value = res.data.previewUrl || '';
...
async function handleLicenseUpload(option: any) {
  const { file } = option;
  try {
    const res = await uploadFile(file, 'MERCHANT_LICENSE');
    if (res && res.data) {
      form.value.businessLicense = res.data.resourceId as any;
      licensePreviewUrl.value = res.data.previewUrl || '';
...
const submitForm = () => {
  merchantApplyFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      ...
      form.value = { ...initFormData } as MerchantApplyForm;
